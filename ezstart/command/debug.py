...
class DebugCommand(commands.Cog):
    ...
    # @commands.check(maindb.kinsi_check)
    async def _eval(self, ctx):
        try:
            if ctx.author.id not in admin: return await ctx.send(alldata.cmd_error_msg)
            _last_result = None
            env = {'bot': self.bot, 'ctx': ctx, 'channel': ctx.channel, 'author': ctx.author, 'guild': ctx.guild,
                   'message': ctx.message, '_': _last_result, "db": self.db,
                   "self": self
                   }
            env.update(globals())
            body = self.cleanup_code(ctx.message.content[6:].lstrip())
            stdout = io.StringIO()
                        ...
                        if len(value) >= limith:
                            ...
                            while True:
                                ...
                                try:
                                    ...
                                    return await mes.add_reaction(batu)
                                if guess.content == "x": return await mes.add_reaction(batu)
                                try:
