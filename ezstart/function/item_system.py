...
class System:
    ...
    async def get_item_list(self, ctx, user_id, pg, max_pg):
        try:
            ...
            my_items = await self.db.fetch("SELECT item_id, count FROM item WHERE user_id=$1 and "
                                           "item_id >= $2 and item_id <= $3 ORDER BY item_id",
    ...
    async def get_material_list(self, ctx, user_id, pg, max_pg):
        try:
            username = self.bot.get_user(user_id).name
            my_materials = await self.db.fetch("SELECT material_id, count FROM material WHERE user_id=$1 ORDER BY material_id", (user_id,))
    ...
    async def get_skill_list(self, ctx, user_id, pg, max_pg):
        try:
            username = self.bot.get_user(user_id).name
            my_skills = await self.db.fetch("SELECT skill_id, count FROM skill WHERE user_id=$1 ORDER BY skill_id", (user_id,))
    ...
    async def fish(self, ctx, user_id, user_id2=None):
        ...
        if not user_id2:
            user_id2 = user_id
        battle = await self.db.fetchrow("SELECT channel_id FROM channel_join WHERE user_id=$1", (user_id2,))
        if not battle:
            embed = discord.Embed(description=f"{user_name}さん、<@{user_id2}>さんは戦闘に参加していませんよ？")
            return await ctx.reply(embed=embed)
        if not await self.db.consume_item(user_id, 10000, 1):
            embed = discord.Embed(description=f"{user_name}さんはそのアイテムを持っていません！")
            return await ctx.reply(embed=embed)
        player_hp = await self.db.player_max_hp(user_id2)
        await self.db.execute("UPDATE channel_join SET hp=$1 WHERE user_id=$2", (player_hp, user_id2))
        embed = discord.Embed(description=f"{user_name}はお魚様の力を使い<@{user_id2}>を全回復させた！\n(短縮形: f)")
        return await ctx.reply(embed=embed)
    ...
    async def potion_effect(self, ctx, user_id, item_id, effect_id, level, count, add_text=""):
        ...
        embed = discord.Embed(description=f"{user_name}は{alldata.effects[effect_id]}Lv.{level}({count}ターン)を付与した！{add_text}")
        return await ctx.reply(embed=embed)
